...
`.github/workflows/validate-published-apt.yml` exercises the public HTTPS repository on clean hosted Ubuntu, including archive fingerprint verification, `apt update`, candidate discovery, install, reinstall with user-configuration preservation, and uninstall. A separate job in the same workflow executes the one-command bootstrap itself against the public repository. The initial v0.1.0 public path also completed real WSL2 install/reinstall/remove validation.

For v0.1.2, the first real previous-version package-upgrade proof was completed in a separate disposable Ubuntu 26.04 WSL2 distribution against the signed public repository. Published v0.1.1 installed successfully, APT upgraded it to published v0.1.2, the CLI reported the expected versions, representative user-owned configuration remained byte-for-byte unchanged, and guarded Bash/Zsh startup files remained byte-for-byte unchanged through prerequisite package installation, install, upgrade, reinstall, and package removal.

Future WSL/WSL2 release package validation must use an explicitly disposable distribution/home. Do not use a maintainer's normal WSL distro or normal home for package-upgrade proof; preserve validation evidence and keep the disposable target available for review until the result is reconciled.
